proc template;
	...
	source StatGraph.Correlation.PACFNORMPlot;
	define statgraph StatGraph.Correlation.PACFNORMPlot;
   		dynamic title series nlags _byline_ _bytitle_ _byfootnote_;
   		begingraph;
      		entrytitle title;
      		layout overlay / yaxisopts=(label="Standardized PACF") xaxisopts=(linearopts=(integer=
         			true) label="Lag");
	         	...
	         	needleplot x=eval (lags(series,max=nlags)) y=eval (pacfnorm(series, nlags=nlags)) /
	            		   rolename=(xtipname=eval (lags(series,max=nlags)) ytipname=eval (
	            	 	   pacfnorm(series, nlags=nlags))) tiplabel=(xtipname="Lag" ytipname=
	            		   "Standardized PACF") tip=(xtipname ytipname) lineattrs=(color=
